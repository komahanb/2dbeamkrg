...
subroutine EV_F(N, X, NEW_X, F, IDAT, DAT, IERR)
  ...
  !call  Krigingestimate(ndimin,ndimint,xavgin,xstdin,fctin,fctindxin,DATIN,initpts,ncyc,nptsin,statin,probtypeIN,flagin,fmeanout,fvarout,fmeanprimeout,fvarprimeout)

  call Krigingestimate(N,N,x,sigmax,10,0,DAT(1001:1020),60,30,60,0,probtype,myflag,fmeantmp,fvartmp,fmeanprimetmp,fvarprimetmp)
...
subroutine EV_G(N, X, NEW_X, M, G, IDAT, DAT, IERR)
  ...
  do i=1,M
     ...
     call Krigingestimate(N,N,x,sigmax,10,i,DAT(1001:1020),60,30,60,0,probtype,myflag,fmeantmp,fvartmp,fmeanprimetmp,fvarprimetmp)
